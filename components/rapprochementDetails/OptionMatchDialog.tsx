...
export default function CreateOptionMatchDialog({ releve }: { releve: any }) {
	...
	const [nonRapprochesGrandLivres, setNonRapprochesGrandLivres] = useState<
		any[]
	>([]);
		...
		refetch()
			.then((result) => {
				if (result.data) {
					...
					setError(
						"Une erreur est survenue lors du chargement des grands livres.",
					);
				}
			})
	...
	const filteredGrandLivres = nonRapprochesGrandLivres.filter(
		(item) =>
			...
			<Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
				<DialogTrigger asChild>
					<Button
						size="sm"
						className={
							"bg-blue-600 rounded-sm my-2 hover:bg-blue-700 text-white"
						}
					>
						<Equal size={14} />
					</Button>
					...
					<DialogDescription>
						<div className="space-y-4">
							...
							<div>
								...
								{new Date(detailItem?.date_ecriture).toLocaleDateString()}
							</div>
							<div>
								<span className="font-semibold">Montant:</span>{" "}
								{detailItem?.debit
									? `-${formatMontant(detailItem.debit)}`
									: formatMontant(detailItem?.credit)}
							</div>
							<div>
								<span className="font-semibold">Compte:</span>{" "}
								{detailItem?.compte || detailItem?.cpte_alt || "N/A"}
							</div>
						</div>
					</DialogDescription>
				</DialogContent>
